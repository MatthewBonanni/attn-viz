...
export function mlaUpprojGraph(p) {
    ...
    return {
        ...
        tensors: [
            ...
            { id: 'scores', shape: [B, n_h, S, S],       label: 'QKᵀ',    stage: 6, row: 0, color: '#9b59b6', dimNames: ['B','n_h','S','S'],
              desc: `Attention scores: Q' @ [K; k_r']^T. Decomposes as (q_content @ K^T) + (q_rope @ k_r'^T). The first term captures content similarity, the second encodes relative position.` },
            { id: 'mask',   shape: [S, S],                label: 'Mask',   stage: 6, row: 1, color: '#1abc9c', dimNames: ['S','S'], type: 'mask',
              desc: 'Causal mask.' },
            { id: 'attn',   shape: [B, n_h, S, S],       label: 'Attn',   stage: 8, row: 0, color: '#9b59b6', dimNames: ['B','n_h','S','S'],
              desc: 'Attention weights.' },
            { id: 'ctx',    shape: [B, S, D],             label: 'Ctx',    stage: 12, row: 1, color: '#e67e22', dimNames: ['B','S','D'],
              desc: 'Context vectors, concatenated across heads.' },
            { id: 'W_O',    shape: [D, D],                label: 'Wo',     stage: 12, row: 2, color: '#7b68ee', dimNames: ['D','D'], type: 'weight',
              desc: 'Output projection.' },
            { id: 'out',    shape: [B, S, D],             label: 'Out',    stage: 13, row: 1, color: '#3498db', dimNames: ['B','S','D'],
              desc: 'Attention output.' },
        ],
        ops: [
            ...
            { id: 'qkt',        type: 'matmul',     inputs: ['Q_r','K','k_r'],   output: 'scores', label: 'Q @ Kᵀ',
              desc: `Attention scores with decoupled RoPE: score = (q_content @ K^T) + (q_rope @ k_r'^T). Content similarity and relative position are computed separately and summed.` },
            { id: 'masking',    type: 'mask',       inputs: ['scores','mask'],   output: 'attn',   label: 'Mask+Softmax',
              desc: 'Apply causal mask then softmax.' },
            { id: 'attn_v',     type: 'matmul',     inputs: ['attn','V'],        output: 'ctx',    label: 'Attn @ V',
              desc: 'Weighted sum of values.' },
...
export function mlaAbsorbedGraph(p) {
    ...
    return {
        ...
        tensors: [
            ...
            { id: 'W_QR',     shape: [d_c, dr],             label: 'W_qr',   stage: 3, row: 1, color: '#7b68ee', dimNames: ['d_c','d_r'], type: 'weight', badge: 'ABSORBED',
            ...
            { id: 'q_rp',     shape: [B, n_h, S, dr],      label: 'q_r',    stage: 4, row: -1, color: '#ff7043', dimNames: ['B','n_h','S','d_r'],
              desc: `Pre-RoPE query component, projected from c_q via W_qr.` },
            { id: 'q_r',      shape: [B, n_h, S, dr],      label: "q_r'",   stage: 5, row: -1, color: '#ff7043', dimNames: ['B','n_h','S','d_r'],
              desc: `RoPE query — after applying rotary position embedding to q_r.` },
            // Scores
            { id: 'scores',   shape: [B, n_h, S, S],       label: 'QKᵀ',    stage: 6, row: 0, color: '#9b59b6', dimNames: ['B','n_h','S','S'],
              desc: `Attention scores = (q' @ c_kv^T) + (q_r' @ k_r'^T). Content similarity (latent space) plus relative position (RoPE). The two terms are added to get final scores.` },
            { id: 'mask',     shape: [S, S],                label: 'Mask',   stage: 6, row: 1, color: '#1abc9c', dimNames: ['S','S'], type: 'mask',
              desc: 'Causal mask.' },
            { id: 'attn',     shape: [B, n_h, S, S],       label: 'Attn',   stage: 8, row: 0, color: '#9b59b6', dimNames: ['B','n_h','S','S'],
              desc: 'Attention weights.' },
            // Latent context: attn @ c_kv (still in latent space)
            { id: 'c_ctx',    shape: [B, n_h, S, d_c],     label: 'c_ctx',  stage: 10, row: 1, color: '#e67e22', dimNames: ['B','n_h','S','d_c'], badge: 'LATENT',
            ...
            { id: 'W_UV',     shape: [d_c, d_h],            label: 'W↑v',    stage: 10, row: 2, color: '#7b68ee', dimNames: ['d_c','d_h'], type: 'weight',
        ...
        ops: [
            ...
            { id: 'latent_qk',    type: 'matmul',     inputs: ['q_lat','c_KV','q_r','k_r'], output: 'scores', label: "Scores",
              desc: `scores = (q' @ c_kv^T) + (q_r' @ k_r'^T). First term: content-based attention in d_c-dimensional latent space. Second term: position-based attention via decoupled RoPE in d_r-dimensional space. Both computed without expanding to d_h.` },
            { id: 'masking',      type: 'mask',       inputs: ['scores','mask'],     output: 'attn',   label: 'Mask+Softmax',
              desc: 'Apply causal mask then softmax.' },
