...
export function mlaUpprojGraph(p) {
    ...
    return {
        ...
        tensors: [
            ...
            { id: 'c_Q',      shape: [S_q, d_c],        label: 'c_q',     stage: 2, row: 0, color: '#c0392b', dimNames: [lq,'d_c'],
            ...
            { id: 'Q_flat', shape: [S_q, D],             label: 'Q_flat',  stage: 5, row: 0, color: '#e74c3c', dimNames: [lq,'D'],
              desc: `Decompressed query result before splitting into heads.` },
            { id: 'K_flat', shape: [S, D],              label: 'K_flat',  stage: 5, row: 1, color: '#2ecc71', dimNames: [ls,'D'],
              desc: `Decompressed content keys before splitting into heads.` },
            { id: 'V_flat', shape: [S, D],              label: 'V_flat',  stage: 5, row: 2, color: '#f39c12', dimNames: [ls,'D'],
              desc: `Decompressed values before splitting into heads.` },
            { id: 'Q',      shape: [n_h, S_q, d_h],   label: 'Q',       stage: 6, row: 0, color: '#e74c3c', dimNames: ['n_h',lq,'d_h'],
            ...
            { id: 'V',      shape: [n_h, S, d_h],     label: 'V',       stage: 6, row: 2, color: '#f39c12', dimNames: ['n_h',ls,'d_h'],
              desc: 'Decompressed values in per-head form.' },
            { id: 'Q_r',    shape: [n_h, S_q, d_h],   label: "Q'",      stage: 7, row: 0, color: '#e74c3c', dimNames: ['n_h',lq,'d_h'],
            ...
            { id: 's_rope', shape: [n_h, S_q, S],     label: "q_rk_rᵀ", stage: 9, row: 2, color: '#9b59b6', dimNames: ['n_h',lq,ls],
            ...
            { id: 'attn_head', shape: [n_h, S_q, d_h], label: 'AV_head', stage: 14, row: 1, color: '#e67e22', dimNames: ['n_h',lq,'d_h'],
              desc: `Per-head context vectors: weighted sum of values, before merging across heads.` },
        ...
        ops: [
            { id: 'compress_q',  type: 'compress',   inputs: ['X','W_DQ'],        output: 'c_Q',      label: 'Down-proj',
              desc: `Compress queries: X @ W↓q → c_q [S_q, ${d_c}].` },
            ...
            { id: 'decomp_q',   type: 'decompress', inputs: ['c_Q','W_UQ'],      output: 'Q_flat',   label: 'Up-proj Q',
              desc: `Decompress queries: c_q @ W↑q → [S_q, D].` },
            { id: 'decomp_k',   type: 'decompress', inputs: ['c_KV','W_UK'],     output: 'K_flat',   label: 'Up-proj K',
              desc: `Decompress content keys from full cache: c_kv @ W↑k → [S, D].` },
            { id: 'decomp_v',   type: 'decompress', inputs: ['c_KV','W_UV'],     output: 'V_flat',   label: 'Up-proj V',
              desc: `Decompress values from full cache: c_kv @ W↑v → [S, D].` },
            { id: 'view_q',     type: 'reshape',    inputs: ['Q_flat'],           output: 'Q',        label: 'View',
              desc: `Reshape query: [S_q, D] → [n_h, S_q, d_h]. Splits D=${D} into n_h=${n_h} × d_h=${d_h}. Zero-cost metadata operation.` },
            { id: 'view_k',     type: 'reshape',    inputs: ['K_flat'],           output: 'K',        label: 'View',
              desc: `Reshape key: [S, D] → [n_h, S, d_h]. Zero-cost metadata operation.` },
            { id: 'view_v',     type: 'reshape',    inputs: ['V_flat'],           output: 'V',        label: 'View',
              desc: `Reshape value: [S, D] → [n_h, S, d_h]. Zero-cost metadata operation.` },
            { id: 'rope_q',     type: 'rope',       inputs: ['Q'],               output: 'Q_r',      label: 'RoPE',
    ...
    return {
        ...
        ops: [
            ...
            { id: 'absorbed_proj', type: 'matmul',     inputs: ['c_Q','W_QK'],        output: 'q_lat',    label: 'Absorbed proj',
              desc: `Content query in latent space: c_q @ W_QK → q' [n_h, S_q, d_c]. Replaces separate Q/K up-projections.` },
            { id: 'rope_q_proj',   type: 'matmul',     inputs: ['c_Q','W_QR'],        output: 'q_rp',     label: 'Linear',
              desc: `Project c_q to RoPE query space: c_q @ W_qr → q_r [n_h, S_q, ${dr}].` },
            ...
            { id: 'latent_attn_v',type: 'matmul',     inputs: ['attn','c_KV'],       output: 'c_ctx',    label: 'Attn @ c_kv', stage: 12,
              routeBelow: ['c_KV'],
              desc: `Latent attention: Attn @ c_kv → c_ctx [n_h, S_q, d_c]. Weighted sum in latent space.` },
